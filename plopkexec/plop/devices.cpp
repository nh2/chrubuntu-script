...
bool Device::Mount(const char *dev, bool is_cdrom)
{
    ...
    } else for(std::vector<string>::iterator supportedFS = Filesystems.begin(); supportedFS != Filesystems.end(); supportedFS++) {
    //for (int i = 0; i < NUM_SUPPORTED_FS; i++)
	//std::cout<<*Iter<<endl;

        if (!mount (dev, "/mnt", (*supportedFS).c_str(), MS_RDONLY, ""))
        {
            Log ("Mounted %s, %s (ro)", dev,  (*supportedFS).c_str());
            return true;
        }
    }
